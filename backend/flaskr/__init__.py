...
def create_app(test_config=None):
    ...
    def post_question():
        ...
        if (
          (new_question == '') or (new_answer == '') or
          (new_difficulty == '') or (new_category == '')):
            abort(422)
    ...
    def search_questions():
        ...
        if search_term == '':
            abort(422)
        try:
            result_set = Question.query.filter(Question.question.ilike(
                                f'%{search_term}%')).all()
            if (len(result_set) == 0):
                abort(404)
    ...
    def play_quiz_question():
        try:
            ...
            random_question = available_questions[random.randint(
                                0,
                                len(available_questions)-1)]
